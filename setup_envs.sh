...
# setup_envs.sh
# Creates .env files for all services with local development configuration.
...
echo "📝 Creating ai/broker_interviewer/.env"
cat > ai/broker_interviewer/.env <<EOF
...
# 4. Embedding Service
echo "📝 Creating ai/embedding/.env"
cat > ai/embedding/.env <<EOF
DATABASE_HOST=localhost
DATABASE_PORT=5433
DATABASE_USER=admin
DATABASE_PASSWORD=password
...
# 5. WhatsApp API
echo "📝 Creating ai/whatsApp_api/.env"
cat > ai/whatsApp_api/.env <<EOF
